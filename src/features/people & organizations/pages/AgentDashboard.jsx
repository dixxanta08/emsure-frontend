...
const AgentDashboard = () => {
  ...
  const agentId = loggedInUser?.agentId; // Assuming agentId is available in loggedInUser

  const { data: dashboardData, isLoading } = useQuery({
    queryKey: ["agentDashboard", agentId],
    queryFn: () => dashboardService.getAgentDashboard(agentId),
  ...
  if (!dashboardData || !agentId) {
    return (
      <div className="p-8 text-center text-gray-600">
        No data available or agent not authenticated
      </div>
    );
  }
