...
import { Form, Modal, Tabs, Tooltip, Popconfirm } from "antd"; // Added Popconfirm import
...
const SubscribedPlanDetail = () => {
  const fields = [
    ...
    { name: "numberOfUsers", label: "Number of Users" },
  ...
  const columns = [
    ...
    { accessorKey: "employeeStartDate", header: "Start Date" },
    {
      id: "actions",
      header: "Actions",
      cell: ({ row }) => (
  ...
  const claimsColumns = [
    ...
    { accessorKey: "claimantName", header: "Claimant Name" },
  ...
  const mutation = useMutation({
    ...
    onSuccess: (response) => {
      toast({
        variant: "success",
        title: "Employee Subscription updated successfully",
        description: "Employee Subscription has been updated successfully.",
      });
    ...
    onError: (error) => {
      console.error("Failed to update employee subscription:", error);
      const errorMessage =
        error.response?.data?.message ||
        "There was a problem while updating the subscription.";
      toast({
        variant: "destructive",
  ...
  const handleModalSelectSearch = (searchTerm) => {
    setModalEmployeeSearchTerm(searchTerm);
  };
  ...
  const {
    ...
    enabled: !!planSubscriptionData?.companyId,
    retry: false,
  });
      ...
      children: (
        <>
          ...
          <AppDataTable
            ...
            onRowClick={(row) =>
              navigate(
                `/subscription-management/plan-subscriptions/${subscriptionId}/employees/${row.employeeId}`
              )
            }
          />
        </>
      ...
      children: (
        <>
          ...
          <AppDataTable
            ...
            error={isSubscriptionClaimsError}
            onRowClick={(row) =>
    ...
    <div className="p-8 bg-[#fafafa] h-full">
      <div className="bg-white p-4">
        ...
        <div className="p-4 mt-4 rounded-md flex flex-col gap-8 mb-4">
          <div className="w-full grid grid-cols-2 gap-4">
            <div>
              ...
              <Link
                to={`/insurance-management/plans/${planSubscriptionData?.planId}/details`}
              >
            ...
            <div>
              ...
              <Link
                to={`/people-organizations/companies/${planSubscriptionData?.companyId}/details`}
              >
                <p className="text-purple-500 hover:text-purple-800 underline font-semibold">
                  {planSubscriptionData?.companyName}
                </p>
              </Link>
            </div>
            <div>
        ...
        <div className="flex items-center gap-4 m-auto justify-center w-full my-8">
          {!isAgent && planSubscriptionData?.isActive === "Active" && (
            <Popconfirm
