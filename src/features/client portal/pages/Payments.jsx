...
import { Select, DatePicker, Button, Modal, Descriptions } from "antd";
import { format } from "date-fns";

const Payments = () => {
  const { loggedInUser } = useAuth();
  const isEmployer = useMemo(() => loggedInUser?.roleName === "EMPLOYER", [loggedInUser]);
  const companyId = useMemo(() => loggedInUser?.companyId, [loggedInUser]);
  ...
  const { data: payments, isLoading } = useQuery({
    queryKey: ["payments", pageIndex, pageSize, searchTerm, paymentType, paymentStatus, dateRange, isEmployer, employeeId],
    queryFn: async () => {
      let response;
      if (isEmployer) {
        // For employers, get all payments for their company
        response = await unifiedPaymentService.getCompanyPayments(
          companyId,
          pageIndex,
          pageSize,
          searchTerm
        ...
        // For employees, get only their payments
        response = await unifiedPaymentService.getEmployeePayments(
          employeeId,
          pageIndex,
          pageSize,
          searchTerm
        ...
        // For admin or other roles, get all payments
        response = await unifiedPaymentService.getAllPayments(
          pageIndex,
          pageSize,
          searchTerm
        );
      ...
      if (dateRange) {
        filteredPayments = filteredPayments.filter((payment) => {
          ...
          return (
            paymentDate >= dateRange[0].startOf("day").toDate() &&
            paymentDate <= dateRange[1].endOf("day").toDate()
          );
        });
      }
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="flex flex-wrap gap-4 mb-6">
          <div className="flex-1 min-w-[200px]">
            <SearchBar onSearchClick={setSearchTerm} />
          </div>
          ...
          >
            ...
            <Select.Option value="employee_subscription">Subscription Payment</Select.Option>
          </Select>
        ...
        <AppDataTable
          ...
          pagination={{
            pageIndex,
            pageSize,
            totalItems: payments?.totalItems || 0,
            totalPages: payments?.totalPages || 0,
          ...
          {selectedPayment && (
            <Descriptions bordered column={2}>
              ...
              <Descriptions.Item label="Payment Type">
                {selectedPayment.paymentType === "claim" ? "Claim Payment" : "Subscription Payment"}
              </Descriptions.Item>
